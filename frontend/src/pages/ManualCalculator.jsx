...
const TaxModule = () => {
      ...
      try {
        ...
        if (res.data && res.data.profile) {
          ...
          setInputs({
            ...
            rentalIncome: prof.rentalIncome ? String(prof.rentalIncome) : '',
            ...
            ppf: prof.ppf ? String(prof.ppf) : '',
            licPremium: prof.licPremium ? String(prof.licPremium) : '',
            tuitionFees: prof.tuitionFees ? String(prof.tuitionFees) : '',
  ...
  const handleAutoFill = async () => {
    try {
      ...
      const exps = res.data;
      let total80D = 0, totalNPS = 0, totalLIC = 0, totalPPF = 0, totalELSS = 0;
      exps.forEach(exp => {
        const t = (exp.title + ' ' + exp.category).toLowerCase();
        if (t.includes('health') || t.includes('insurance') || t.includes('mediclaim')) {
          if (!t.includes('life') && !t.includes('lic')) total80D += exp.amount;
        }
        if (t.includes('nps') || t.includes('pension')) totalNPS += exp.amount;
        if (t.includes('lic') || t.includes('life insurance')) totalLIC += exp.amount;
        if (t.includes('ppf') || t.includes('provident fund')) totalPPF += exp.amount;
        if (t.includes('elss') || t.includes('mutual fund')) totalELSS += exp.amount;
      });
      setInputs(prev => ({
        ...prev,
        deductions80D: total80D > 0 ? String(total80D) : prev.deductions80D,
        nps: totalNPS > 0 ? String(totalNPS) : prev.nps,
        licPremium: totalLIC > 0 ? String(totalLIC) : prev.licPremium,
        ppf: totalPPF > 0 ? String(totalPPF) : prev.ppf,
        elss: totalELSS > 0 ? String(totalELSS) : prev.elss
      }));
    } catch (err) {
      console.error(err);
    }
  ...
  const handleCalculate = async (e) => {
    ...
    const body = {
      ...
      ppf: Number(inputs.ppf || 0),
      licPremium: Number(inputs.licPremium || 0),
      tuitionFees: Number(inputs.tuitionFees || 0),
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 p-6 rounded-lg shadow-sm h-fit">
            <div className="flex justify-between items-center border-b border-gray-100 dark:border-slate-700/50 pb-3 mb-4">
              ...
              <button 
                ...
                className="text-xs bg-indigo-50 text-indigo-600 hover:bg-indigo-100 font-semibold px-3 py-1.5 rounded-md transition"
              >
                Auto-fill from Expenses
              </button>
            </div>
            ...
            <form onSubmit={handleCalculate} className="space-y-4">
              <div className="bg-gray-50 dark:bg-slate-900 border border-gray-150 dark:border-slate-700 rounded-md p-3.5 space-y-3.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-600 dark:text-slate-300 uppercase mb-1">Rental Income</label>
                    <input
                      type="text"
                      name="rentalIncome"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-gray-600 dark:text-slate-300 uppercase mb-1">Other Source Revenues</label>
                    <input
                      type="text"
                      name="otherIncome"
              ...
              <div className="bg-green-50/50 border border-green-100 rounded-md p-3.5 space-y-3.5">
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-600 dark:text-slate-300 uppercase mb-1">Sec 80C (Other)</label>
                    <input
                      type="text"
                      name="deductions80C"
                      placeholder="e.g. 120000"
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-gray-600 dark:text-slate-300 uppercase mb-1">PPF</label>
                    <input
                      type="text"
                      name="ppf"
                      placeholder="e.g. 50000"
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-600 dark:text-slate-300 uppercase mb-1">LIC Premium</label>
                    <input
                      type="text"
                      name="licPremium"
                      placeholder="e.g. 20000"
                      className="w-full px-3 py-1.5 border border-gray-300 dark:border-slate-600 rounded-md text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 bg-white dark:bg-slate-800"
                      value={inputs.licPremium}
                      onChange={handleChange}
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-600 dark:text-slate-300 uppercase mb-1">Tuition Fees</label>
                    <input
                      type="text"
                      name="tuitionFees"
            ...
            {!calcs ? (
              ...
              <div className="space-y-6" id="tax-result-cards">
                ...
                {suggestions && suggestions.length > 0 && (
                  <div className="bg-white dark:bg-slate-800 border border-indigo-100 rounded-lg p-5 shadow-sm space-y-4">
                    <h4 className="font-bold text-indigo-900 flex items-center gap-2 border-b border-indigo-50 pb-2">
                      <Info className="w-5 h-5 text-indigo-600" />
                      Smart Tax Saving Opportunities
                    </h4>
                    <div className="space-y-3">
                      {suggestions.map((sug, idx) => (
                        <div key={idx} className="flex items-start gap-3 p-3 bg-indigo-50 rounded-md">
                          <div className={`mt-0.5 w-2 h-2 rounded-full shrink-0 ${sug.priority === 'high' ? 'bg-red-500' : sug.priority === 'medium' ? 'bg-yellow-500' : 'bg-blue-500'}`}></div>
                          <div>
                            <h5 className="font-semibold text-sm text-indigo-900">{sug.title}</h5>
                            <p className="text-xs text-indigo-700 mt-0.5 leading-relaxed">{sug.description}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <div className="border border-gray-200 dark:border-slate-700 rounded-lg p-5 space-y-4 shadow-sm hover:border-gray-300 transition">
                    ...
                    <div className="space-y-2 text-xs">
                      ...
                      <div className="border-t border-gray-150 dark:border-slate-700 pt-2 flex justify-between">
